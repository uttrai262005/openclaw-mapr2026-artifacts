...
BT_DIR = WS / "dataset_clean" / "BT3"
RUBRIC_PATH = WS / "rubric" / "rubric_BT3.docx"
OUT_PATH = Path(os.environ.get("OUT_PATH", str(WS / "output" / "ket_qua_BT3.xlsx")))
OUT_PATH_RESUME = WS / "output" / "ket_qua_BT3_resume.xlsx"
...
def call_llm_once(url: str, token: str, model: str, rubric_text: str, submission_text: str) -> Dict[str, Any]:
    ...
    user = (
        "RUBRIC (trích từ rubric_BT3.docx):\n"
        f"{rubric_text}\n\n"
        "CV CỦA SINH VIÊN (text trích xuất; có thể gồm 2 file, đã ghép lại):\n"
        f"{submission_text}\n\n"
...
def group_files_by_mssv(files: List[Path]) -> Dict[str, List[Path]]:
    """Group BT3 submissions by MSSV.

    After regrouping, files are stored under dataset_clean/BT3/<MSSV>/...
    So prefer the parent folder name when it looks like an MSSV.
    """
    groups: Dict[str, List[Path]] = {}
    for p in files:
        parent = p.parent.name
        if re.fullmatch(r"[34]\d{6}", parent):
            m = parent
        else:
            m = extract_mssv(p)
    ...
    for m in list(groups.keys()):
        groups[m] = sorted(groups[m], key=lambda x: x.name.lower())
    return groups


def save_wb(wb, out_path: Path) -> Path:
    """Try saving workbook. If target is locked, fall back to resume / unique file."""
    try:
        wb.save(out_path)
        return out_path
    ...
    # fallback 1: resume
    try:
        wb.save(OUT_PATH_RESUME)
        return OUT_PATH_RESUME
    except PermissionError:
        pass

    # fallback 2: unique timestamped file
    import time

    ts = time.strftime('%Y%m%d_%H%M%S')
    unique = out_path.with_name(out_path.stem + f'_{ts}' + out_path.suffix)
    wb.save(unique)
    return unique
...
def main():
    if not BT_DIR.exists():
        raise SystemExit(f"BT3 dir not found: {BT_DIR}")
    ...
    all_files = sorted([p for p in BT_DIR.rglob('*') if p.is_file() and p.suffix.lower() in {'.pdf','.docx','.txt','.md'}])
    groups = group_files_by_mssv(all_files)

    mssv_list = sorted(groups.keys())
    print(f"BT3 files: {len(all_files)}")
    print(f"BT3 unique MSSV groups: {len(mssv_list)}")
    ...
    for i, mssv in enumerate(mssv_list, 1):
        ...
        paths = groups[mssv]
        # Read + concatenate all files for this MSSV
        try:
            parts: List[str] = []
            for p in paths:
                t = read_submission_text(p)
                t = (t or '').strip()
                if t:
                    parts.append(f"===== FILE: {p.name} =====\n{t}")
            sub_text = "\n\n".join(parts).strip()
            if not sub_text or len(sub_text) < 20:
            ...
            out_path = save_wb(wb, out_path)
            recorded_errors += 1
            print(f"[{i}/{len(mssv_list)}] {mssv}: READ ERROR -> recorded")
            continue

        # Exactly ONE API call per MSSV.
        try:
            ...
            print(f"[{i}/{len(mssv_list)}] {mssv} ({label}): OK total={total}")
        except Exception:
            if r is None:
                ws.append([mssv, "", "", "", "", "", "", "", "Lỗi"])
                idx[mssv] = ws.max_row
            ...
            out_path = save_wb(wb, out_path)
            recorded_errors += 1
            print(f"[{i}/{len(mssv_list)}] {mssv}: API/JSON ERROR -> recorded")
