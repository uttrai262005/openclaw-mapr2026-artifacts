...
BT_DIR = WS / "dataset_clean" / "BT1"
RUBRIC_PATH = WS / "rubric" / "rubric_BT1.docx"
OUT_PATH = Path(os.environ.get("OUT_PATH", str(WS / "output" / "ket_qua_BT1.xlsx")))
...
def call_llm_once(url: str, token: str, model: str, rubric_text: str, submission_text: str) -> Dict[str, Any]:
    ...
    user = (
        "RUBRIC (trích từ rubric_BT1.docx):\n"
        f"{rubric_text}\n\n"
...
def main():
    if not BT_DIR.exists():
        raise SystemExit(f"BT1 dir not found: {BT_DIR}")
    ...
    for i, path in enumerate(files, 1):
        ...
        try:
            ...
            tc1 = round_quarter(clamp(float(res.get("tc1", 0.0)), 0.0, MAX_TC))
            tc2 = round_quarter(clamp(float(res.get("tc2", 0.0)), 0.0, MAX_TC))
            tc3 = round_quarter(clamp(float(res.get("tc3", 0.0)), 0.0, MAX_TC))
            tc4 = round_quarter(clamp(float(res.get("tc4", 0.0)), 0.0, MAX_TC))
            tc5 = round_quarter(clamp(float(res.get("tc5", 0.0)), 0.0, MAX_TC))
